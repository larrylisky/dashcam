...
//  dashcam
//
//  Created by Larry Li on 4/20/20.
...
//==========================================================================
class Dashcam : NSObject, UIGestureRecognizerDelegate {
    ...
    open var hasFlash: Bool {
        get {
            for device in AVCaptureDevice.devices() {
                if device.hasMediaType(.video) && device.hasFlash {
                    return true
                }
            }
            return false
    ...
    private var _frontCameraDevice: AVCaptureDevice? {
        for device in AVCaptureDevice.devices() {
            if device.hasMediaType(.video) && device.position == .front {
                return device
            }
        }
        return nil
    ...
    private var _backCameraDevice: AVCaptureDevice? {
        for device in AVCaptureDevice.devices() {
    ...
    */
    ...
    private func _setupCamera(_ completion: @escaping () -> Void) {
        ...
        do {
            var audioOptions : AVAudioSession.CategoryOptions
            if #available(iOS 10.0, *) {
                audioOptions = [.mixWithOthers, .defaultToSpeaker, .allowBluetoothA2DP, .allowAirPlay]
            } else {
                audioOptions = [.mixWithOthers, .defaultToSpeaker]
            }
            try audioSession.setCategory(.playAndRecord, options: audioOptions )
...
///////////////////////////////////////////////////////////////////////////////
extension Dashcam : AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
